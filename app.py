...
source_category = st.sidebar.selectbox(
    "Select Source", ["Original Datasets", "Generated Mails"]
)
...
if source_category == "Original Datasets":
    ...
    if not raw_generated_files:
        ...
        selected_display_name = st.sidebar.selectbox(
            "Select File", list(display_map.keys())
        )
...
if selected_filename:
    ...
    if not email_ids:
        st.error(
            f"Error: No data found in '{selected_filename}'. Please check the file."
        )
        st.stop()
...
with c3:
    t = st.selectbox(
        "Tone",
        ["Friendly", "Sympathetic", "Professional", "Diplomatic", "Witty"],
...
if any(metrics.values()):
    ...
    with m1:
        st.info(f"**Faithfulness**\n\n{metrics['faithfulness']}")
    with m2:
        st.info(f"**Completeness**\n\n{metrics['completeness']}")
    with m3:
        st.info(f"**Robustness**\n\n{metrics['robustness']}")
    with m4:
        st.info(f"**Professionalism**\n\n{metrics['professionalism']}")
    ...
    with m5:
        st.error(
            f"**URL Preservation (Defect Check)**\n\n{metrics['url_preservation']}"
        )
    with m6:
        st.warning(
            f"**Scope Accuracy (Selection Check)**\n\n{metrics['scope_accuracy']}"
        )
